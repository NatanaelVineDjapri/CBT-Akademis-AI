...
function JawabanEssayCard({
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await periksaEssayAdminUniversitas(ujianId, pesertaId, penilaian);
      await globalMutate(`/ujian/admin-universitas/hasil/${ujianId}`);
      setIsEditing(false);
      onSaved();
    } catch {
  ...
  const handleReset = async () => {
    ...
    setSaveError("");
    try {
      await resetEssayAdminUniversitas(ujianId, pesertaId);
      await globalMutate(`/ujian/admin-universitas/hasil/${ujianId}`);
          ...
          <div key={row.id} className="border border-gray-100 rounded-xl p-4">
            ...
            {showInputs ? (
              ...
              <div className="rounded-lg px-3 py-2 text-xs text-gray-700 border border-dashed border-gray-300">
                <span className="font-medium text-gray-500">Catatan: </span>{row.dosen_feedback}
              </div>
            ) : null}
          </div>
        ))}
