      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
          ...
          <div className="flex items-center gap-3">
            ...
            {data && (
              <span className="text-xs font-semibold px-3 py-1 rounded-full text-white"
                ...
                Total Peserta: {data.info.total_peserta}
              </span>
            )}
          </div>
        ...
        <div className="overflow-x-auto">
                    ...
                    <td className="px-4 py-4">
                      {p.nilai !== null ? (
                        <span className="font-semibold"
                          style={{ color: p.lulus ? "var(--color-primary)" : "var(--color-danger, #ef4444)" }}>
                          {p.nilai}/100
                        </span>
                      ) : (
          ...
          {data && pesertaFiltered.length === 0 && (
            <EmptyState message="Tidak ada peserta ditemukan." />
          )}
        </div>
      </div>
                    ...
                    {allOpsiKeys.map(k => {
                      ...
                      return (
                        <td key={k} className="px-4 py-4 text-sm font-medium"
                          ...
                          {count > 0 ? `${count} peserta` : <span className="text-gray-300">-</span>}
                        </td>
                      );
                    })}
                    <td className="px-4 py-4">
