      ...
      <div className="flex-1">
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden px-3 pb-2">
          ...
          <table className="w-full text-sm table-fixed">
            ...
            <thead>
              <tr className="border-b border-gray-100">
                <th className="text-left text-xs text-gray-400 font-bold px-4 py-3">#</th>
                    ...
                    <tr key={item.id} className="border-b border-gray-50 hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-4 py-4 font-medium truncate">
                        <Link
                          ...
                          style={{ color: "var(--color-primary)" }}
                          onMouseEnter={() =>
                            preload(`/ujian/admin-universitas/hasil/${item.id}`, () =>
                              getDetailUjianAdminUniversitas(item.id),
                            )
                          }
                        >
                          {item.nama_ujian}
          ...
          {!showSkeleton && data && items.length === 0 && (
            <EmptyState message="Belum ada data ujian PMB." flat />
          )}
        </div>
      </div>
