...
export default function AdminMonitoringDetailPage({ params }: { params: Promise<{ ujianId: string }> }) {
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        ...
        {(ujian || ujianMeta) && (
          <div className="px-5 py-3 border-b border-gray-100 bg-gray-50 flex items-center gap-5 flex-wrap">
            ...
            {aktif > 0 && (
              <Link
                href={`/admin-universitas/monitoring/${slug}/live-view`}
              ...
              {paged.length === 0 ? (
                ...
                <tr key={p.peserta_ujian_id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-5 py-3 font-medium text-gray-800">
                    <Link href={`/admin-universitas/monitoring/${slug}/${toSlug(p.nama ?? String(p.user_id))}?pid=${p.peserta_ujian_id}&uid=${p.user_id}&eid=${id}`} className="hover:underline" style={{ color: "var(--color-primary)" }}>
                      {p.nama ?? "-"}
                    </Link>
                  </td>
