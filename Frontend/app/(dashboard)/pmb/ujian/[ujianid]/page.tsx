...
export default function UjianPmbDetailPage({ params }: { params: Promise<{ ujianid: string }> }) {
  ...
  const handleAutoSelesai = async () => {
    ...
    try { await selesaikanUjian(session.peserta_ujian_id); } finally { window.location.href = "/pmb/ujian?tab=selesai"; }
  };

  const handleSelesai = async () => {
    if (!session) return;
    selesaiRef.current = true; setConfirmSelesai(false); setPhase("submitting");
    try { await selesaikanUjian(session.peserta_ujian_id); window.location.href = "/pmb/ujian?tab=selesai"; }
  ...
  if (phase === "error") return (
    <div className="flex flex-col items-center justify-center h-64 gap-3">
      ...
      <button onClick={() => router.push("/pmb/ujian")} className="text-xs text-gray-400 hover:underline">Kembali</button>
    </div>
  );
    ...
    <div className="flex items-center justify-center min-h-[60vh]">
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8 w-full max-w-sm flex flex-col gap-4">
        <div>
          ...
          <p className="text-xs text-gray-400 mt-1">Ujian ini memerlukan kode akses.</p>
        </div>
        ...
        <button onClick={() => router.push("/pmb/ujian")} className="text-xs text-gray-400 hover:underline text-center">Batal</button>
      </div>
    </div>
  );
      ...
      {confirmSelesai && (
        <ConfirmModal
          ...
          message={belumJawab > 0
            ? `Masih ada ${belumJawab} soal yang belum dijawab. Yakin ingin mengumpulkan jawaban?`
