...
import { getAdminAkademisAudits, type AuditModel, type AuditEvent } from "@/services/AuditService";
...
export default function AdminAkademisLogPage() {
  ...
  const { data: univData } = useSWR("/universitas/all-log", () => getUniversitas({ per_page: 200 }), { revalidateOnFocus: false });
  const univList = univData?.data ?? [];

  const { data, isLoading } = useSWR(
    ["/audit/global", filterModel, filterEvent, filterUniv, debouncedSearch, page, perPage, sortBy, sortDir],
    () => getAdminAkademisAudits({
      ...(filterModel ? { model: filterModel } : {}),
      ...(filterEvent ? { event: filterEvent } : {}),
      ...(filterUniv ? { universitas_id: Number(filterUniv) } : {}),
      ...
      <div className="flex-1 bg-white rounded-2xl border border-gray-100 overflow-hidden flex flex-col">
        <div className="px-7 pt-6 pb-3 flex items-center justify-between flex-wrap gap-3 shrink-0">
          ...
          <div className="flex items-center gap-2.5 flex-wrap">
            <select
              value={filterUniv}
              onChange={(e) => handleFilter(() => setFilterUniv(e.target.value))}
              ...
              {isLoading ? (
                Array.from({ length: perPage }).map((_, i) => (
                  <tr key={i}>
                    ...
                    <td className="py-3.5 px-1.5 border-b border-gray-100"><div className="h-3 bg-gray-100 rounded-full w-16 animate-pulse" /></td>
                    <td className="py-3.5 px-1.5 border-b border-gray-100"><div className="h-3 bg-gray-100 rounded-full w-28 animate-pulse" /></td>
                  </tr>
                ))
              ) : !data?.data.length ? (
                <tr>
                  <td colSpan={5} className="py-10 text-center text-sm text-gray-400">Tidak ada log ditemukan.</td>
                </tr>
              ) : (
                data.data.map((log, i) => (
                  <tr key={log.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className={`py-3.5 px-1.5 ${i < data.data.length - 1 ? "border-b border-gray-100" : ""}`}>
                      <span className="text-xs text-gray-500">{log.user?.universitas_kode ?? "-"}</span>
                    </td>
                    <td className={`py-3.5 px-1.5 ${i < data.data.length - 1 ? "border-b border-gray-100" : ""}`}>
