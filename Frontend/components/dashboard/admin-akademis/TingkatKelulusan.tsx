...
import { getAdminAkademisKelulusan } from "@/services/DashboardServices";
...
function CustomTooltip({ active, payload }: any) {
  ...
  return (
    <div className="bg-white border border-gray-100 rounded-xl px-3 py-2 shadow-sm text-xs max-w-[180px]">
      ...
      <p className="text-gray-400">{d.total > 0 ? `${d.lulus} / ${d.total} peserta` : "Belum ada data"}</p>
    </div>
  );
}
...
export default function TingkatKelulusan() {
  const { data, isLoading, error } = useSWR(
    "/dashboard/admin-akademis/kelulusan",
    getAdminAkademisKelulusan,
    ...
    return (
      <div className="bg-white rounded-2xl border border-gray-100 p-5">
        <div className="flex items-center gap-2 mb-3">
          ...
          <span className="text-sm font-medium text-gray-800">Tingkat Kelulusan per Universitas</span>
        </div>
