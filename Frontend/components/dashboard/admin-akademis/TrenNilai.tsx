...
import { getAdminAkademisTrenNilai } from "@/services/DashboardServices";
...
export default function TrenNilai() {
  const { data, isLoading } = useSWR(
    "/dashboard/admin-akademis/tren-nilai",
    getAdminAkademisTrenNilai,
  ...
  if (data.length === 0) {
    return (
      <div className="bg-white rounded-2xl border border-gray-100 p-5">
        ...
        <div className="min-h-[180px] flex items-center justify-center"><EmptyState flat size={64} message="Belum ada data nilai dalam 6 bulan terakhir." /></div>
      </div>
    );
  }
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 p-5 flex flex-col h-full">
      <div className="flex items-center gap-2 mb-4 shrink-0">
        <TrendingUp size={15} className="text-gray-500" />
        <span className="text-sm font-medium text-gray-800">Tren Rata-rata Nilai Platform (6 Bulan)</span>
      </div>
