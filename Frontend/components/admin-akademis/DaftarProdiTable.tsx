...
import { getProdi, deleteProdi, ProdiItem } from "@/services/ProdiService";
import { useDebounce } from "@/hooks/useDebounce";
import Pagination from "@/components/filtering/Pagination";

interface Props {
  fakultasId: number;
  onEdit: (item: ProdiItem) => void;
  onAdd: () => void;
  refreshKey: number;
}
...
export default function DaftarProdiTable({ fakultasId, onEdit, onAdd, refreshKey }: Props) {
  ...
  const { data, isLoading, mutate } = useSWR(
    ["/prodi", fakultasId, debouncedSearch, page, refreshKey],
    ...
    <>
      <div className="bg-white rounded-2xl border border-gray-100 flex flex-col overflow-hidden">
        ...
        <div className="px-6 pt-5 pb-3 flex items-center justify-between gap-3 flex-wrap shrink-0">
          <span className="text-sm font-bold" style={{ color: "var(--color-primary)" }}>Daftar Program Studi</span>
          ...
          <table className="w-full border-collapse">
            <thead>
              <tr className="border-b border-gray-200">
                ...
                <th className="text-left text-xs font-semibold text-gray-500 pb-2.5">Program Studi</th>
                <th className="text-left text-xs font-semibold text-gray-500 pb-2.5 w-24 text-center">Mahasiswa</th>
                <th className="text-left text-xs font-semibold text-gray-500 pb-2.5 w-24 text-center">Dosen</th>
            ...
            <tbody>
              {isLoading ? (
                  ...
                  <tr key={i}>
                    {Array.from({ length: 5 }).map((_, j) => (
                      <td key={j} className="py-3.5 border-b border-gray-100">
                        <div className="h-3 bg-gray-100 rounded-full animate-pulse" style={{ width: j === 0 ? 24 : j === 4 ? 48 : "70%" }} />
                      </td>
                    ))}
                  </tr>
                ))
                ...
                <tr>
                  <td colSpan={5} className="py-10 text-center text-sm text-gray-400">Belum ada program studi terdaftar.</td>
                </tr>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 px-4">
          <div className="bg-white rounded-2xl w-full max-w-sm shadow-xl overflow-hidden">
            <div className="flex items-center justify-between px-6 py-4" style={{ backgroundColor: "var(--color-primary)" }}>
              <h3 className="text-base font-bold text-white">Hapus Program Studi</h3>
            ...
            <div className="px-6 py-5">
              <p className="text-sm text-gray-600">
                Yakin ingin menghapus program studi <span className="font-semibold text-gray-800">"{deleteTarget.nama}"</span>? Tindakan ini tidak dapat dibatalkan.
              </p>
            </div>
