...
interface Props {
  onEdit: (item: UniversitasItem) => void;
  onAdd: () => void;
  refreshKey: number;
}
...
export default function DaftarUniversitasTable({ onEdit, onAdd, refreshKey }: Props) {
  ...
  const { data, isLoading, mutate } = useSWR(
    ["/universitas", debouncedSearch, page, refreshKey],
    ...
    <>
      <div className="bg-white rounded-2xl border border-gray-100 flex flex-col overflow-hidden">
        ...
        <div className="px-6 pt-5 pb-3 flex items-center justify-between gap-3 flex-wrap shrink-0">
          <span className="text-sm font-bold" style={{ color: "var(--color-primary)" }}>Daftar Universitas</span>
          ...
          <table className="w-full border-collapse">
            ...
            <tbody>
              {isLoading ? (
                  ...
                  <tr key={i}>
                    {Array.from({ length: 7 }).map((_, j) => (
                      <td key={j} className="py-3.5 border-b border-gray-100">
                        <div className="h-3 bg-gray-100 rounded-full animate-pulse" style={{ width: j === 0 ? 24 : j === 6 ? 48 : "70%" }} />
                      </td>
                    ))}
                  </tr>
                ))
                ...
                <tr>
                  <td colSpan={7} className="py-10 text-center text-sm text-gray-400">Belum ada universitas terdaftar.</td>
                </tr>
                ...
                return (
                  ...
                  >
                      ...
                      <div className="flex items-center gap-2.5">
                        {item.logo ? (
                          <img
                            ...
                            alt={item.nama}
                            className="w-7 h-7 rounded-md object-contain border border-gray-100 shrink-0"
                          />
                        ) : (
                    ...
                    <td className={`text-xs text-gray-500 py-3.5 pr-3 ${!isLast ? "border-b border-gray-100" : ""}`}>
                      <span className="line-clamp-2">{item.alamat || "-"}</span>
                    </td>
                    <td className={`text-sm text-gray-700 font-medium py-3.5 text-center ${!isLast ? "border-b border-gray-100" : ""}`}>{item.fakultas_count ?? 0}</td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 px-4">
          <div className="bg-white rounded-2xl w-full max-w-sm shadow-xl overflow-hidden">
            <div className="flex items-center justify-between px-6 py-4" style={{ backgroundColor: "var(--color-primary)" }}>
              <h3 className="text-base font-bold text-white">Hapus Universitas</h3>
            ...
            <div className="px-6 py-5">
              <p className="text-sm text-gray-600">
                Yakin ingin menghapus universitas <span className="font-semibold text-gray-800">"{deleteTarget.nama}"</span>? Tindakan ini tidak dapat dibatalkan.
              </p>
            </div>
