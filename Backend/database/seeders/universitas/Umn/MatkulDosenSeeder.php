...
class MatkulDosenSeeder extends Seeder
{
    ...
    {
        $dosens = DB::table('users')->where('role', 'dosen')->where('universitas_id', DB::table('universitas')->where('kode', 'UMN')->value('id'))->pluck('id');
        if ($dosens->isEmpty()) return;
        $matkuls = DB::table('mata_kuliah')->whereIn('prodi_id', DB::table('prodi')->whereIn('kode', ['UTI','USI','UTKE','UMNJ','UAKT','UIK','UJR','UDKV','UDIN','UFLM','UIH'])->pluck('id'))->get();
        $insertData = [];
