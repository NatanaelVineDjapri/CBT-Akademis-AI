...
class BankSoalSeeder extends Seeder
{
    ...
    {
        $dosens = DB::table('users')->where('role', 'dosen')->where('universitas_id', DB::table('universitas')->where('kode', 'BINUS')->value('id'))->get();
        foreach ($dosens as $dosen) {
            for ($i = 0; $i < 3; $i++) {
                ...
                $bankId = DB::table('bank_soal')->insertGetId([
                    ...
                    'bab_id' => $bab?->id,
                    'nama' => "Bank Soal {$matkul->nama} - Set " . ($i + 1),
                    'deskripsi' => "Kumpulan soal untuk " . $matkul->nama,
                    'permission' => $this->permissions[rand(0, 2)],
                    'created_at' => now(),
                    'updated_at' => now(),
                ]);
                ...
                for ($s = 0; $s < $soalCount; $s++) {
                    $tipe = $this->soalTipe[0]; // 60% PG
                    $r = rand(0, 100);
                    if ($r > 60 && $r <= 85) $tipe = $this->soalTipe[1]; // 25% Essay
                    elseif ($r > 85) $tipe = $this->soalTipe[2]; // 15% Checkbox

                    $soalId = DB::table('soal')->insertGetId([
                        'bank_soal_id' => $bankId,
                        'mata_kuliah_id' => $matkul->id,
                        'bab_id' => $bab?->id,
                        'deskripsi' => "Soal {$tipe} nomor " . ($s + 1),
