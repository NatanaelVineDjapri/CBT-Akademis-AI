...
class MatkulDosenSeeder extends Seeder
{
    ...
    {
        $dosens = DB::table('users')->where('role', 'dosen')->where('universitas_id', DB::table('universitas')->where('kode', 'UAJJ')->value('id'))->pluck('id');
        if ($dosens->isEmpty()) return;
        $matkuls = DB::table('mata_kuliah')->whereIn('prodi_id', DB::table('prodi')->whereIn('kode', ['AJTI','AJTE','AJTM','AJTIN','AJMJ','AJAK','AJIH','AJARS'])->pluck('id'))->get();
        $insertData = [];
